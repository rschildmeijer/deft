...
public class JMXDebuggableTimeoutManager implements TimeoutManager,
        TimeoutManagerMXBean {

    private final Logger logger = LoggerFactory
            .getLogger(JMXDebuggableTimeoutManager.class);

    private final TreeMultiset<DecoratedTimeout> timeouts = TreeMultiset
            .create();
    private final Map<SelectableChannel, DecoratedTimeout> index = Maps
            .newHashMap();
    ...
    public void touch(SelectableChannel channel) {
        ...
        DecoratedTimeout oldTimeout = index.get(channel);
        timeouts.remove(oldTimeout);
        Timeout newTimeout = new Timeout(System.currentTimeMillis()
    ...
    public long execute() {
        ...
        TreeMultiset<DecoratedTimeout> defensive = TreeMultiset
                .create(timeouts);
        ...
        return timeouts.isEmpty() ? Long.MAX_VALUE : Math.max(1, timeouts
                .iterator().next().timeout.getTimeout() - now);
    }
