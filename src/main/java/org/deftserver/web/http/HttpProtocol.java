...
//		clientChannel.configureBlocking(false);
		...
		IOLoopFactory.getLoopController().addHandler(clientChannel, this, SelectionKey.OP_READ, new HttpChannelContext(ByteBuffer.allocate(READ_BUFFER_SIZE), id));
	}

	@Override
	public void handleRead(SelectionKey key) throws IOException {
		logger.debug("#id: {} - handle read...{}", ((HttpChannelContext) key.attachment()).getId(),  key);
		SocketChannel clientChannel = (SocketChannel) key.channel();
		HttpRequest request = getHttpRequest(key, clientChannel);
		logger.debug("#id: {} - After get request : {}",  ((HttpChannelContext) key.attachment()).getId(), request);

		if (request!=null && request.isFinished()) {
			logger.debug("#id: {} - Process the complete parsed request.",  ((HttpChannelContext) key.attachment()).getId());
			...
			if (!rh.isMethodAsynchronous(request.getMethod())) {
				response.finish();
				logger.debug("#id: {} - Response finished and closed after synchronous request handler.", ((HttpChannelContext) key.attachment()).getId());
			}else {
...
//					);
				...
				logger.debug("#id: {} - Response not sent after asynchronous request handler.", ((HttpChannelContext) key.attachment()).getId());
			}

		}else {
			logger.debug("#id: {} - Nothing to do since request is null or not completely parsed", ((HttpChannelContext) key.attachment()).getId());
		}
	}
	
	@Override
	public void handleWrite(SelectionKey key) {
		logger.debug("#id: {} - handle write...", ((HttpChannelContext) key.attachment()).getId());
		...
		try {
			...
			logger.debug("#id: {} - sent {} bytes to wire", bytesWritten,ctx.getId());
			if (!toSend.hasRemaining()) {
				logger.debug("#id: {} - sent all data in toSend buffer",ctx.getId());
			...
			logger.error("#id: {} - Failed to send data to client: {}", e.getMessage(), ctx.getId());
	...
	public void closeOrRegisterForRead(SelectionKey key) {
		if (key.isValid() && isKeepAlive(key)) {
			try {
				...
				logger.debug("#id: {} - keep-alive connection. registrating for read.", ((HttpChannelContext) key.attachment()).getId());
			} catch (ClosedChannelException e) {
				logger.debug("#id: {} - ClosedChannelException while registrating key for read: {}", e.getMessage(), ((HttpChannelContext) key.attachment()).getId());
				Closeables.cancelAndCloseQuietly(key);
			}		
		} else {
			// http request should be finished and no 'keep-alive' => close connection
			logger.debug("#id: {} - Closing finished (non keep-alive) http connection", ((HttpChannelContext) key.attachment()).getId()); 
			Closeables.cancelAndCloseQuietly(key);
	...
	private HttpRequest getHttpRequest(SelectionKey key, SocketChannel clientChannel) {
		...
		if (i < 0){
			logger.debug("#id: {} - Nothing to read in this buffer so close it !", ((HttpChannelContext) key.attachment()).getId());
			Closeables.cancelAndCloseQuietly(key);
			return null;
		}
