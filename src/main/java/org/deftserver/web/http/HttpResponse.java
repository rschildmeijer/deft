...
public class HttpResponse {
    ...
    private DynamicByteBuffer responseData = DynamicByteBuffer
            .allocate(WRITE_BUFFER_SIZE);

    public HttpResponse(HttpProtocol protocol, SelectionKey key,
            boolean keepAlive) {
    ...
    public long flush(boolean finish) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("#id : {} - Sending Response chunk :\n{}",
                    ...
                    new String(responseData.array(), 0, responseData.limit()));
        }
        try {
    ...
    public long finish() {
        ...
        logger.debug("#id: {} - Finish and commit HTTP Reponse",
                ((HttpChannelContext) key.attachment()).getId());
    ...
    private String createInitalLineAndHeaders() {
        StringBuilder sb = new StringBuilder(
                HttpUtil.createInitialLine(statusCode));
    ...
    public long write(File file) {
        ...
        try {
            bytesWritten = new RandomAccessFile(file, "r")
                    .getChannel()
                    .transferTo(0, file.length(), (SocketChannel) key.channel());
            logger.debug("sent file, bytes sent: {}", bytesWritten);
        } catch (IOException e) {
            logger.error("Error writing (static file) response: {}",
                    e.getMessage());
        }
        return bytesWritten;
    }
