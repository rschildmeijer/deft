...
public class DynamicByteBufferTest {
    ...
    public void testAllocation() {
        assertInternalState(INITIAL_CAPACITY, 0, INITIAL_CAPACITY,
                INITIAL_CAPACITY);
    }

    private void assertInternalState(int expectedCapacity,
            int expectedPosition, int expectedLimit, int arrayLength) {
    ...
    public void testNoReallactionPut() {
        ...
        dbb.put(data);
        assertInternalState(INITIAL_CAPACITY, data.length, INITIAL_CAPACITY,
                INITIAL_CAPACITY);
    }
    ...
    public void testReallocationLimit() {
        ...
        try {
            ...
            Assert.fail("No exception should be caught on simple put");
        }
    ...
    public void testPrepend() {
        ...
        assertInternalState(expectedCapacity, expectedPosition, expectedLimit,
                arrayLength);

        dbb.put(data);
        expectedCapacity = (int) ((data.length + expectedCapacity) * DynamicByteBuffer.BUFFER_LOAD_FACTOR);
        expectedPosition += data.length;
        expectedLimit = expectedCapacity;
        arrayLength = expectedCapacity;
        assertInternalState(expectedCapacity, expectedPosition, expectedLimit,
                arrayLength);
    }
